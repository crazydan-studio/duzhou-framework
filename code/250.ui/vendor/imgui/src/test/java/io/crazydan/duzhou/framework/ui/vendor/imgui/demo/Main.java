...
public class Main extends Application {
    ...
    private static byte[] loadFromResources(String name) {
        try {
            return Files.readAllBytes(Paths.get(Main.class.getResource("/demo/" + name).toURI()));
    ...
    public void process() {
        if (ImGui.begin("Demo", ImGuiWindowFlags.AlwaysAutoResize)) {
            ...
            ImGui.sameLine();
            ImGui.text(String.valueOf(this.count));
            ImGui.inputText("string", this.str, ImGuiInputTextFlags.CallbackResize);
            ImGui.text("Result: " + this.str.get());
            ImGui.sliderFloat("float", this.flt, 0, 1);
            ImGui.separator();
            ImGui.text("Extra");
            Extra.show(this);
        }
